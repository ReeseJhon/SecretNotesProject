...
def decrypt_notes():
    message_encypted = input_text.get("1.0", END)
    master_secret = master_secret_input.get()

    if len(message_encypted) == 0 or len(master_secret) == 0:
        messagebox.showwarning(title="Error!", message="Please enter all info.")
    else:
        try:
            decrypted_message = decode(master_secret, message_encypted)
            input_text.delete("1.0", END)
...
input_text = Text(width=30, height=10, font=FONT)
input_text.pack()
